...
object Example94 extends IOApp.Simple {
  ...
  val game = new Game[GameState, Command] {
    def init: GameState = GameState(
      people =
        List(Person("🐱", Position(0, 0)), Person("🐰", Position(30, 40))),
      fruit = List(Position(45, 10), Position(50, 50))
    )
    ...
    def action(
      ...
      .eval(stateRef.get)
      .flatMap { state =>
        val person = state.people.head
        val fruit = state.fruit.head
        walk(person.position, fruit, 10.millis)
          .evalMap(position =>
            stateRef.update(GameState.setPosition(_, person.name, position))
          )
          .drain ++ Stream.exec(stateRef.update(GameState.storeFruit(_, fruit)))
      }
